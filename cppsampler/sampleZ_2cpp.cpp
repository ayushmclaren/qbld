...
arma::mat sampleZ_2(arma::mat*y, arma::cube*X, arma::vec beta, arma::cube*S, arma::mat alpha ,double theta, arma::mat*w, double varphi2, double tau2, int m, int n)
{
  ...
  for(int i=0; i<n; i++)
  {
    for(j=0; j<m; j++)
    {
      mean_comp = (((((*X).slice(i)).col(j)).t())*beta + ((((*S).slice(i)).col(j)).t())*alpha.col(i) + theta*(*w)(j,i)).eval()(0,0);
      sd_comp  = tau2*(*w)(j,i);
