...
finalunblock <- function(mc,p,y,datax,datas,x_intercept,s_intercept,b0,B0,c1,d1,burnin)
{
...
## Load Data - no need to load few things again, already in this environment (only for testing)
  ...
  if(s_intercept == FALSE)
    ...
    l = floor(ncol(datas)/n) + 1 #add an intercept column in model matrix                # number of time periods
...
for(nsim in 2:MCMC)
{
  ...
  betap[,nsim] = sampleBeta_2(z,x,s,w,alpha[,,nsim-1],varphi2[nsim-1,1],tau2,theta,invB0, invB0b0,k,m,n)
  
  
  #---------- Sample alpha conditionally on beta,z -------------------
  alpha[,,nsim] = sampleAlpha_fast(z,x,s,betap[,nsim],w,tau2,theta,varphi2[nsim-1,1],l,m,n)
  
  # sampleAlpha(z,x,s,betap[,nsim],w,tau2,theta,varphi2[nsim-1,1],l,m,n)
  ...
  z = sampleZ_2(y,x,s,betap[,nsim],alpha[,,nsim-1],varphi2[nsim,1],w,tau2,theta,m,n)
