...
Let $y$ be the reponse variable, and $z$ be the introduced latent variable as described. The \textbf{QBLD} model can be conveniently expressed in the latent variable formulation (Albert \& Chib, 1993) as follows:
...
$\alpha_{i}$ = $l * 1$ vector of case-specific parameters, and   

$\epsilon_{it}$ = the error term $\overset{\mathrm{iid}}{\sim} AL(0, 1, p)$. 

\bigskip

AL refers to the Asymmteric Laplace Distribution with location, $\mu = 0$, scale $\sigma = 1$, and skew paramter $p$. The error term is decomposed into a normal-exponential mixture representation of the AL distribution, presented in Kozumi and Kobayashi (2011). 

\bigskip
Here, $u_{it} \sim N(0,1)$, is mutually independent of $w_{it} \sim \exp(1)$, where $\exp(.)$ is the exponential distribution. Define:
$\theta = \frac{1-2p}{p(1-p)}$, and $\tau = \sqrt{\frac{2}{p(1-p)}}$. 

Random samples from the AL distribution are generated using \texttt{raldmix} function. (See Appendix) 
...
\bigskip
Building on Eq.(\ref{eq1}),the resulting hierarchical model can be written as: 
...
$IG(.)$ refers to the Inverse-Gamma distribution, $\exp(.)$ refers to the exponential distribution. The starting values for the sampler are sampled from the respective assumed priors, however, one is free to tweak $\beta_{0}$, $B_{0}$, $c1$, and $d1$ values.

\bigskip

\subsection{Blocked vs Unblocked Sampler}
The \textbf{unblocked} version of the Gibbs sampler is faster, but there is potential for poor mixing properties due to correlation between the covariates. I would recommend using ``Unblock" for larger datasets. See Appendix for details of the algorithm.

\bigskip

To avoid potential slow mixing, an alternative \textbf{blocked} algorithm is presented. This algorithm however, takes a longer time to sample the Markov chain. I would recommend using ``Block" for smaller datasets. See Appendix for details of the algorithm.
...
In this example, $wheeze\sim smoking+I(age^2)+age$ translates to response variable, $y_{i}$ = \texttt{wheeze}, and $x_{i}$ as \texttt{smoking, age, age$^2$, $and$ Intercept}.}
...
\item{\texttt{Alpha:} 3-dimensional Matrix (of the form $\mathbb{R}^{k*l*m}$) of MCMC samples of random-effects parameters.}
...
@
Note that the \texttt{foo} attribute is now TRUE, which means the significance on the \texttt{Multi Gelman-Rubin} level have been reached. Note that, in such a case, the summary table for this run will contain the stars unlike the last run.

\bigskip

\texttt{summary} function has the following arguments:
\begin{itemize}
...
\begin{itemize}
\item{\texttt{statistics:} Contains the mean, sd, markov std error, ess and Gelman-Rubin diagnostic}
