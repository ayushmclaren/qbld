...
#raldmix <- function(n,mu=0,sigma=1,p=0.5,plot=FALSE)
#{
...
#  z        = matrix(0,nrow=m,ncol=n) #latent variable
  ...
  #p = 0.25
 # epsilon  = matrix(raldmix(m*n,0,1,p),nrow=m,ncol=n) ### ALD distbn on error terms
...
#mbm <- microbenchmark(old = qbldcpp_f(nsim, p, y_cpp, x_cpp, s_cpp, x_intercept, s_intercept, b0, B0, c1, d1),new = qbldcpp_f(nsim, p, y_cpp, x_cpp, s_cpp, x_intercept, s_intercept, b0, B0, c1, d1),times=1)
#print(mbm)
